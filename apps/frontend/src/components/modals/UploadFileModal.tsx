...
export default function UploadFileModal({
  ...
  const formik = useFormik<UploadFileFormValues>({
    initialValues: {
      name: '',
      size: '',
      type: 'pdf',
  ...
  const setAttachments = (files: FileList | File[]) => {
    ...
    const nextSize = primaryFile ? formatFileSize(primaryFile.size) : '';
    const nextName = primaryFile ? stripFileExtension(primaryFile.name) : '';
    const nextType = primaryFile
      ? getFileTypeFromName(primaryFile.name)
      : 'pdf';
    formik.setFieldValue('name', nextName);
    formik.setFieldValue('type', nextType);
    formik.setFieldValue('size', nextSize);
  ...
  const handleRemoveAttachment = (fileName: string) => {
    ...
    formik.setFieldValue(
      'size',
      nextAttachments[0] ? formatFileSize(nextAttachments[0].size) : '',
    );
    formik.setFieldValue(
      'name',
      nextAttachments[0] ? stripFileExtension(nextAttachments[0].name) : '',
    );
    formik.setFieldValue(
      'type',
      nextAttachments[0] ? getFileTypeFromName(nextAttachments[0].name) : 'pdf',
    );
    if (fileInputRef.current) fileInputRef.current.value = '';
  };
        ...
        <div className="w-full">
          ...
          {attachmentError ? (
            ...
            <p className="mt-2 text-xs text-red-600">
              {String(formik.errors.attachments)}
            </p>
          ) : null}
        </div>
      </div>
