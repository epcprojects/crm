...
import AddContactModal, { type AddContactFormValues } from './AddContactModal';
...
export default function CreateTicketModal({
  ...
  const [createdContacts, setCreatedContacts] = useState<ApiContactOption[]>(
    [],
  );
  ...
  const contactOptions = useMemo(() => {
    const loadedContacts = contactsQuery.data ?? [];
    const options = [
      ...createdContacts.filter(
        (created) =>
          !loadedContacts.some((contact) => contact.id === created.id),
      ),
      ...loadedContacts,
    ].map((contact) => ({
  ...
  const handleCreateContact = async (values: AddContactFormValues) => {
    ...
    setCreatedContacts((current) => [...current, contact]);
    void formik.setFieldValue('contactId', contact.id);
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 divide-x divide-gray-200">
          <div className="space-y-4 p-4 md:p-5">
            ...
            <Dropdown
              label="Contact"
              labelAction={
                canCreateContact && !disableContactSelection ? (
                  <button
                    type="button"
              ...
              placeholder={
                contactsQuery.isLoading ? 'Loading...' : 'Select contact'
              }
      ...
      <AddContactModal
        key="create-lead-add-contact-modal"
